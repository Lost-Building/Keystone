...
function WorldModel({ onSelect }: { onSelect: (label: string) => void }) {
  ...
  useEffect(() => {
    ...
    new GLTFLoader().load('/Keystone/World.glb', (gltf) => {
      ...
      const labels = ['STORE', 'LIBRARY', 'DEALS', 'AVATAR', 'SETTINGS'];
      const cubes = model.getObjectsByProperty('type', 'Mesh').filter((object): object is THREE.Mesh => object instanceof THREE.Mesh).slice(0, labels.length);
    ...
    const hover = (event: PointerEvent) => { if (!model || dragging) return; const rect = canvas.getBoundingClientRect(); const pointer = new THREE.Vector2(((event.clientX - rect.left) / rect.width) * 2 - 1, -((event.clientY - rect.top) / rect.height) * 2 + 1); const raycaster = new THREE.Raycaster(); raycaster.setFromCamera(pointer, camera); const hit = raycaster.intersectObjects(model.getObjectsByProperty('type', 'Mesh').filter((object): object is THREE.Mesh => object instanceof THREE.Mesh), false)[0]; if (highlight) { highlight.parent?.remove(highlight); highlight.geometry.dispose(); (highlight.material as THREE.Material).dispose(); highlight = null; } if (hit && hit.object instanceof THREE.Mesh) { highlight = new THREE.LineSegments(new THREE.EdgesGeometry(hit.object.geometry), new THREE.LineBasicMaterial({ color: 0x66f7ff, depthTest: false, depthWrite: false })); highlight.scale.setScalar(1.012); hit.object.add(highlight); } };
